...
def read_sr(name):
    ...
    return (sr > np.random.random(sr.shape)).astype(np.int)
...
sr = read_sr('Simple2D16relu_L2_64_32_K333_PBC_MReg0.00EReg0.30B1000')
sr_old = read_sr('Simple2D16relu_L3_64_16_16_K3333_MReg0.10EReg0.30B1000_OLD')

nn_list = [calc_nn(x) for x in [mc, sr, sr_old]]
...
plt.ylim((0,350000))
plt.subplot(132)
plt.hist(block_sum(sr).ravel(), bins=5)
plt.ylim((0,350000))
plt.subplot(133)
plt.hist(block_sum(sr_old).ravel(), bins=5)
plt.ylim((0,350000))